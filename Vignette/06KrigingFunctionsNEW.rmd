...
This package is designed to be modular and separate the steps of computation, prediction and simulation. It also uses R S3 and S4 objects methods to simplify the coding. This strategy is especially helpful for different geometries and distance functions. 
Finally, the use of sparse matrix methods is also implemented as S4 methods through the spam package and so much of linear algebra uses the standard R coding even through sparse computations are being done behind the scenes. 

Despite this complexity it is important to keep in mind the computation and statistical results are just the usual ones associated with Kriging and maximum likelihood associated with a Gaussian spatial process model. The difference is  that the spatial  process is specified in a way that is suited to  generating sparse matrices for the computations.  Also the use of objects and methods is just a more deliberate and easier way to create branches in the computations. I.e. if spherical model then do this ... if rectangular domain do this ... . We give some examples in this section. 

## An example of classes and methods

To fix some concepts we give a simple illustration of how a computation can branch to different cases using S3 methods. 
...
{
  out<- list( x= seq( 
    gridInfo$min,  gridInfo$max, gridInfo$delta) )
  return( out)
}
...
{
  out<- list( 
    x = seq( gridInfo$minX,  gridInfo$maxX, gridInfo$delta ),
...
print( out2)
```
The analogy in the LattticeKrig coding is that there are several generic steps in defining the spatial model that benefit from using a method without haing to add many different cases in the top level functions based on geometry. In this example one could just have a line such as   ``` out <- makeGrid( gridInfo)``` and the class of gridInfo would direct what actual function is called. 
...
As mentioned earlier, \ttt{LKrig} is the function that runs all the logic to compute the kriging fit: it calls the functions to create the covariate matrix, basis function matrix, and precision matrix; runs a series of intermediate calculations; calls another function that calculates the estimates for $\mathbf c$ and $\mathbf d$; uses those coefficients to calculate the fitted values and residuals; calls a function that estimates the likelihood of the calculated fit; and finally estimates the effective degrees of freedom in the fitted surface. The idea is that the \ttt{LKinfo} object is the reference for what needs to be done. And the coding at this level does not have explicit branches to different geometries. 
...
```
Creates the correct precision matrix for the basis function coefficients by using the information from  \ttt{LKinfo}. (Here verbose is a TRUE/FALSE value to print output for debugging.)
The code line in \ttt{LKrig} 
```{r eval=FALSE}
G <- t(wX) %*% wX + LKinfo$lambda * (Q)
```
assembles the G matrix from the precision matrix, the value of $\lambda$ and the weighted basis function matrix. Although these matrices  are in sparse format, the reader will note that the coding follows the usual syntax because the spam package has provided methods for addition and multiplication using the usual symbols.
Using the same symbol or function for different classes of data is called overloading. The reader probably already takes for granted in coding say ``` c<- a+b ``` and relying on getting the right result regardless of whether \ttt{a} and \ttt{b} are integers, real numbers or matrices. Creating the different components of the model in LatticeKrig is also an example of overloading with the class being the value of LKgeometry.
...
Model predictions at the data locations are returned in the
\ttt{fitted.values} component of the \ttt{LatticeKrig} and \ttt{LKrig} objects. 
Predicting the fitted curve or surface at arbitrary locations are found but multiplying the new covariate vectors with the \ttt{d.coef} linear model parameters and multiplying the basis functions with the \ttt{c.coef} coefficients. It is probably no surprise that creating the basis functions is determined through the components of the  LKinfo object. 
But with this structure we have a simple form for making predictions and is keeping with other methods in R 
In general  if
...
One complication in this process is the need to evaluate the predictions on a grid of covariates. For example for the Colorado climate example one may want to predict just the smooth function of climate based on latitude and longitude or add to it the linear model adjustment due to elevation.
Moreover, one might want to evaluate these on a grid to make it easier to plot the results on a map. In 2 or more dimensions keeping track of the grids adds a step to this process and the reader is referred to the help files and the Quick Start example for more details. 

## Simulation 

A feature of the LatticeKrig model is that it is efficient to simulate realizations of the process. This operation referred to *unconditional simulation* can be then used to generate conditional simulations to determine approximate prediction standard errors and quantify the estimate's uncertainty. 
