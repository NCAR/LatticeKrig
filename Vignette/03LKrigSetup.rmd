...
The parameter \ttt{nlevel} is an integer that determines the number of different lattice sizes the computation should run on. This is set to 3 by default in \ttt{LatticeKrig}. Increasing \ttt{nlevel} will increase the potential detail of the fitted surface, and will increase the computation time significantly. The coefficients at each different lattice size is computed independently, and the resulting coefficients are scaled by the weights in \ttt{alpha}. The following plot shows the basis functions for each of the three default levels.
...
legend("topright", c("First Level", "Second Level", "Third Level"), col=c("black", "red", "green"), lty=1)
...
The following plots show how different values of \ttt{NC}, \ttt{a.wght}, \ttt{alpha}, and \ttt{nu} affect the covariance function. These plots are all one dimensional for ease of viewing; the covariance function is radially symmetric in higher dimensions so these plots can easily be extended to higher dimensions.
...
ALPlot = cbind(basis(abs(xGrid/2.5)), 1/2 * basis(abs(xGrid/2.5)), 1/4 * basis(abs(xGrid/1.25)))
matplot(ALPlot, type="l", lty=1, x = xGrid, xlab = "Location", ylab = "Covariance",
