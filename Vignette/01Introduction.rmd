...
* \ttt{LatticeKrig}: Top level function that sets up the default spatial model and estimates some covariance parameters, then calls \ttt{LKrig}.
* \ttt{LKrig}: Performs the Kriging computation for a fixed LatticeKrig model.
* \ttt{LKrigSetup}: Creates an \ttt{LKinfo} object to store the parameters to use for a \ttt{LatticeKrig} / \ttt{LKrig} call; especially useful for examining the effect of changing one parameter on the fit.
