...
At least one of \ttt{alpha}, \ttt{alphaObject}, and \ttt{nu} must be set. In most cases you will use \ttt{alpha} or \ttt{nu}. The parameter \ttt{alpha} should be a vector of length \ttt{nlevel} that holds the weights that scale the basis functions on each different lattice size; \ttt{nu} is a scalar that controls how quickly the values in \ttt{alpha} decay. When \ttt{nu} is set, \ttt{alpha} will be filled by setting \ttt{alpha[i] = 2\^(-2 * i * nu)}, then scaling so the sum of the weights in \ttt{alpha} is 1. This scaling should always be done before passing in \ttt{alpha} to make sure the model fits correctly.
...
xGrid = seq(-5,5, length.out = 400)
NCPlot = cbind(basis(abs(xGrid/1.25)), basis(abs(xGrid/2.5)), basis(abs(xGrid/5)))
matplot(NCPlot, type="l", lty=1, x = xGrid, xlab = "Location", ylab = "Covariance",
        main="Effect of NC on covariance function")
legend("topright", c("NC = 5", "NC = 10", "NC = 20"))
...
ALPlot = cbind(basis(abs(xGrid/2.5)), 0.5 * basis(abs(xGrid/2.5)), 0.25 * basis(abs(xGrid/2.5)))
matplot(NCPlot, type="l", lty=1, x = xGrid, xlab = "Location", ylab = "Covariance",
        main="Effect of alpha on covariance function")
legend("topright", c("alpha = 1", "alpha = 0.5", "alpha = 0.25"))

NuPlot = cbind(1/3 * basis(abs(xGrid/10)), 1/7 * basis(abs(xGrid/10)), 1/21 * basis(abs(xGrid/10)))
matplot(NuPlot, type="l", lty=1, x = xGrid, xlab = "Location", ylab = "Covariance",
        main="Effect of nu on covariance function (3rd level)")
legend("topright", c("nu = 0", "nu = 0.5", "nu = 1"))
```