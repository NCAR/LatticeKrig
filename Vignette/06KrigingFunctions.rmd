...
The \ttt{LKinfo} object also has a second class that is the geometry. This controls how other components in this object are filled in. In particular the component \ttt{LatticeInfo} will change, reflecting different lattices based on the geometries. See \ttt{help(LKGeometry)} for more details on what needs to done for a given geometry. 
