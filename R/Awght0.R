# ####################################################################################
LKrigFindLambdaAwght0 <- function(x, y, ...,  LKinfo,
                                 use.cholesky=NULL, 
                                 ...
                                 maxit=15,
                                 verbose=FALSE) {
  # this is an unconstrained MLE finder for a single a.wght and lambda. 
  #
  #require(stats)
  ...
  # check that a.wght is just a single value
   if( !attr( LKrigArgs$LKinfo$a.wght,"isotropic")){
     stop("Not a single a.wght parameter.")
   }
 ...
 #                     upper=c(upperBoundLogLambda,upperBoundOmega), 
  ...
  dimnames(capture.evaluations)<- list( NULL,
                                        c("lambda","logLambda", "a.wght","Omega", "rho.MLE", "sigma.MLE","lnProfileLike.FULL"))
